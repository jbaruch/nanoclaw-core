...
The contract starts from the travel copy's and tightens it on review: `{"available": true, "tz", "local_now", "local_date"}` or the all-null unavailable shape, `home_tz` never a fallback. The script now also expresses the current instant in the resolved zone (`local_now`, `local_date`, pinnable with `--now` for tests) so no caller reproduces the `astimezone` conversion inline. Exit codes separate the two kinds of "nothing": `0` with `available: false` is a store that was read and holds no usable zone; `1` is a store that could not be read at all (missing file, no table, corruption, permission), which a scheduling caller treats as a hard failure while a surface still degrades on the stdout shape. A caller that must pick a zone when unavailable uses the container `$TZ`, which is the operator's zone at spawn since `jbaruch/nanoclaw#954`. `rules/temporal-awareness.md` now points at the script instead of a hand-written SQL read. Tests ported from the travel copy and extended; `pyrightconfig.json` gains the skill root.
